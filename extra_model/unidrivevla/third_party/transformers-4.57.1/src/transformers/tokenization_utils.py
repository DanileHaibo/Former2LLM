    ...
    def _add_tokens(self, new_tokens: Union[List[str], List[AddedToken]], special_tokens: bool = False) -> int:
        ...
        for token in new_tokens:
            ...
            if token in self._added_tokens_decoder:
                continue
            token_is_special = getattr(token, "special", False)
            if not token_is_special and token.normalized and getattr(self, "do_lower_case", False):
            ...
            if token_is_special and str(token) not in self.all_special_tokens:
